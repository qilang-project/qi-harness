...
奇语标准库提供 `标准库.MCP服务器`（qi 可作为 **MCP 服务器**：创建/注册工具·资源·提示/启动，含 `列出工具`/`执行工具`）。
qi-harness 目前**未集成 MCP 客户端**（连接外部 MCP server 把其工具拉进 agent 工具循环）——这是 roadmap。需要时可在 qi 工具的 `处理` 函数里调 `标准库.MCP服务器` 的本进程工具。
