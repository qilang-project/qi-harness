...
- 用户问 qi-harness 的会话、对话、流式、工具调用、MCP 客户端、结构化输出、预算、handoff、向量记忆/检索、关系记忆（图记忆）、图控制流/断点续跑/HITL、评估、护栏、追踪
...
| 路 | 模块 | 召回依据 | 答什么 |
| --- | --- | --- | --- |
| 词法 | `Harness.记忆` | SQLite `LIKE`，逐词匹配内容 + 关键词 | 关于 X 我记过什么 |
| 语义 | `Harness.向量记忆` | embedding 余弦相似度 | 跟这句话意思相近的 |
| 关系 | `Harness.图记忆` | 属性图邻域 / 最短路径 | 谁用过 X、X 依赖什么、A 和 B 怎么连的 |

三路输出**同一种行格式**（`-【标签】正文`），可以直接拼成一个块注入提示词；没命中一律返回 `""`。

```qi
导入 Harness.图记忆::{ 打开图记忆, 记实体, 关联, 关联带属性, 图回忆,
                      怎么连的, 按关系回忆, 图记忆条数, 图记忆统计, 关闭图记忆 };

变量 库 = 打开图记忆("/tmp/agent关系.kv");
记实体(库, "mcp:fetch", "工具");            // 幂等
关联(库, "研究员", "用过", "mcp:fetch");     // 两端实体不存在会自动建；同端点同关系幂等
关联(库, "mcp:fetch", "能取", "网页");

图回忆(库, "研究员", 5);          // 一跳双向邻域 → -【用过】研究员 → mcp:fetch
图回忆(库, "研究员", 2, 5);       // N 跳（按元数重载）
怎么连的(库, "研究员", "网页");    // -【路径】研究员 → mcp:fetch → 网页
按关系回忆(库, "用过", 5);         // 这个关系下的所有边
关闭图记忆(库);
```

接进代理：`启用图记忆(代理值, 路径)` / `记关联(代理值, 主体, 关系, 客体)` / `代理图记忆库(代理值)`。
之后 `带记忆运行` 把词法块和关系块合并注入同一个 `## 相关记忆与过往经验` 标题块
（事件分别是 `memory_recall` 与 `graph_recall`），两路都可选，都没命中就一个字不加。

名字里的冒号（`mcp:fetch`、`用户:1`）内部转成安全键段（qi-graph 的键靠 `:` 分段），
原名存进节点/边属性 —— 查找过安全名、展示回原名，调用方全程只见原名。

> ⚠ `图记忆` 需要 **qi-graph（包名 `Graph`）+ qi-kv（包名 `KV`）** 在场，
> 所以它**故意不从 `Harness.qi` re-export**（同 `观测台` 之于 qi-web），要显式
> `导入 Harness.图记忆::{...}`。不用这个模块的程序完全不受影响。
> 别跟 `Harness.图` 混：那个是控制流图（节点=函数、边=跳转），这个是数据图（节点=实体、边=关系）。
