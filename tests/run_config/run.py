...
def main() -> int:
    for source in (ROOT / "运行配置.qi", ROOT / "代理.qi"):
        subprocess.run(["qi", "check", str(source)], cwd=ROOT, check=True)

    with tempfile.TemporaryDirectory(prefix="qi-run-config-") as temporary:
        ...
        try:
            ...
            if not port_file.exists():
                raise RuntimeError("timed out waiting for run-config fixture")
            port = int(port_file.read_text(encoding="utf-8"))
            env = os.environ.copy()
            env["QI_TEST_URL"] = f"http://127.0.0.1:{port}"
            completed = subprocess.run(
                [str(ROOT / "tests" / "run-qi-test.sh"), str(TEST_DIR / "run_config_test.qi")],
                cwd=ROOT,
                env=env,
                timeout=30,
                check=False,
