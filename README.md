...
qi-harness `0.5.x` requires Qi `2026.09.12-2` or newer（qi 2.0 那一轮：`列出目录` 回字符串列表句柄、`JSON.取显示值`、放开的保留字）。CI and release preflight pin the exact Qi, qi-runtime, qi-gui, and qi-web commits used for this release and run compile/link probes for every required ABI family.
...
导入 Harness::{
    ...
    创建代理, 添加工具, 运行, 简单问, 带图问, 流式问, 流式运行, 关闭代理,
    工具
};
...
// 带图问答（多模态，需视觉模型）：图像URL 支持 https:// 与 data:...;base64,。
// 与 简单问 同一外壳（重试/预算/真实用量/持久会话）；带图消息进会话历史，
// 之后纯文本 简单问/运行 还能追问这张图。
变量 看图 = 带图问(代理值, "这张图什么颜色？", "https://example.com/图.png");
...
到这里为止两路召回都在问「哪条记忆跟这个词/这句话像」。有一类问题它们答不了：

- 谁用过 `mcp:fetch`？
- 报告器依赖什么？
- 研究员和网页之间是怎么连上的？

答案不在任何一条记忆的字面里，而在**条目之间的边**上。`图记忆` 把这些边显式存下来
（qi-graph 属性图，落在一个 qi-kv 文件里），于是可以顺着边走。

```qi
导入 Harness.图记忆::{打开图记忆, 记实体, 关联, 图回忆, 怎么连的, 按关系回忆, 关闭图记忆};

变量 库: 整数 = 打开图记忆("/tmp/agent关系.kv");
记实体(库, "mcp:fetch", "工具");
关联(库, "研究员", "用过", "mcp:fetch");     // 两端实体不存在会自动建
关联(库, "mcp:fetch", "能取", "网页");
关联(库, "研究员", "用过", "mcp:fetch");     // 幂等：同一对端点 + 同一关系不会长出第二条

打印行(图回忆(库, "研究员", 5));            // 一跳邻域
// -【用过】研究员 → mcp:fetch

打印行(图回忆(库, "研究员", 2, 5));          // N 跳（按元数重载）
// -【用过】研究员 → mcp:fetch
// -【能取】mcp:fetch → 网页
...
打印行(按关系回忆(库, "用过", 5));           // 这个关系下的所有边
关闭图记忆(库);
```

召回走**双向**邻域：出边答「X 依赖什么」，入边答「谁用过 X」。`图回忆` 的关键词逗号分隔
（中英文逗号都认，跟 `回忆` 同款），多个关键词的邻域合并去重，`条数上限` 封顶；没命中返回 `""`。

**三路召回的输出是同一种行格式**（`-【标签】正文`），所以可以直接拼成一个块：

| 路 | 模块 | 召回依据 | 答什么 |
| --- | --- | --- | --- |
| 词法 | `记忆.qi` | SQLite `LIKE`，逐词匹配内容 + 关键词 | 关于 X 我记过什么 |
| 语义 | `向量记忆.qi` | embedding 余弦相似度 | 跟这句话意思相近的 |
| 关系 | `图记忆.qi` | 属性图邻域 / 最短路径 | 谁用过 X、X 依赖什么、A 和 B 怎么连的 |

接进代理后 `带记忆运行` 会把词法块和关系块合并注入同一个标题块（各自可选，都没命中就一个字不加）：

```qi
导入 Harness.图记忆::{启用图记忆, 记关联};

代理值 = 启用记忆(代理值, "/tmp/记忆.db");      // 词法
代理值 = 启用图记忆(代理值, "/tmp/关系.kv");    // 关系
记关联(代理值, "研究员", "用过", "mcp:fetch");
带记忆运行(代理值, "帮我查点东西", "研究员");
```

注入的提示长这样（`memory_recall` + `graph_recall` 两条事件各记一路）：
...
**名字里的冒号**：qi-graph 的键靠 `:` 分段，而 agent 世界的名字常带冒号（`mcp:fetch`、`用户:1`）。
内部走安全名（能当键段的原样，不能的整串转十六进制），**原名存进节点/边属性**，
查找时过安全名、展示时读回原名 —— 调用方全程只见原名。

> ⚠ 这一路需要 **qi-graph（包名 `Graph`）+ qi-kv（包名 `KV`）** 在场。
> 它们只被 `图记忆.qi` 这一个模块用到，所以**它故意不从 `Harness.qi` re-export**
> （跟依赖 qi-web 的 `观测台` 同样的道理）：只要主入口引一下它的符号，每个用
> Harness 的程序哪怕只调 `创建代理` 都得能解析到那两个包才编得过。不碰这个模块的人
> 完全不受影响。同理 `代理.qi` 那边只存一个不透明句柄槽 + 一个召回钩子，
> 接线在 `图记忆.qi` 里做（依赖方向是 图记忆 → 代理，不是反过来）。

演示（无需 LLM）：`qi run examples/图记忆_关系召回.qi`

注意别跟 `图.qi` 混：那个是**控制流**图（LangGraph-lite，节点是函数、边是条件跳转），
...
## 嵌进别的程序时（2026-08-10 实测）

把 agent 包成一个「被别的进程调用、用 stdout 交付结果」的可执行体时，有三件事要先处理。
背景是给 [eval-go/agentbench](https://github.com/liliang-cn/eval-go) 写基准适配器（约 780 行 Qi，框架零改动）。

**1. legacy trace 直写 stdout，会污染协议。** 非流式 `运行` 默认把 `llm_call`/`retry`/`usage`
的 JSON 打到 stdout。调用方若按行解析 stdout，这些行会混进结果。双保险：
...
外加 wrapper 把整个进程的 stdout 重定向到 stderr，真正的结果写文件、由 wrapper 打出来——
这样任何一层漏网的打印都伤不到协议。

**2. 采集工具调用序列不必动 lifecycle 总线。** `工具` 结构体的 `处理` 就是普通
`函数(字符串):字符串`，在每个 handler 入口 append 一行到文件，拿到的就是真实派发顺序
（含并行 tool_calls 的实际次序），比订阅事件再解析 JSONL 简单也更可靠。

**3. 批量执行先编译。** `qi run` 每次重编译约 2.5 秒；跑 50 次就是白烧两分多钟。
先 `qi compile` 出二进制（staging 后原子 `mv`），后续直接 exec。`qi compile` 会在源码
旁落一个 `.o`，记得清。

**4. 本地依赖直接在 `[依赖]` 里写路径。** 适配器目录的 `qi.toml` 写：

```toml
[依赖]
Harness = "../qi-harness"          # 相对 qi.toml 所在目录；绝对路径同样可以
```

别名必须是 `Harness`（等于 qi-harness 的 `[包] 名称`）。适配器放哪儿都行，不需要
挨着 qi-harness，也不用建符号链接——2026-08-10 之前那行声明不起作用、只能靠目录布局，
符号链接是当时的绕法，现在删掉即可。详见 qi-lang `references/包管理.md`。
