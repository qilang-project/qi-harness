...
- **意图识别**（新模块 `意图.qi`，书 14.5）：`创建分类器` / `加类别` / `加子类别` /
  `加示例` / `加边界示例` / `加背景` / `设兜底` / `分类提示` / `识别意图` / `多级识别`，
  外加两个现成的 —— `展示型分类器`（关系图/表格/图表/地图）与 `问题种类分类器`
  （数据相关 / 系统相关{文档|模式} / 反馈与抱怨）。按书里列的六条要求组装提示词
  （任务说明、类别定义、例子、**边界例子**、兜底、理由字段）。
- **文本转查询**（新模块 `文本转查询.qi`，书 14.7）：`创建查询提示` / `用模式` /
  `用意图` / `加查询示例` / `加图注记` / `记上次失败` / `组装提示`（八段结构）/
  `生成查询`，以及推理顺序那一组 `推理在前` / `答案在前` / `按顺序排字段` /
  `是推理在前` / `查询字段`。核心是**字段顺序即思考顺序**：生成任务排
  关系 → 推理 → 查询，分类任务反过来排 类别 → 理由（书里明说分类是例外）。
  模式当字符串传入，本模块不依赖 qi-graph。
- `examples/仓库问答.qi`：拿 cpa 的 gemini + t2m 的 embeddinggemma 跑通整条链
  （LLM 抽事实建图 → 实体归并 → 中心性 → 意图分岔 → 生成查询 → 真答案 / 语义兜底）。
- 图记忆召回**按相关度挑**：`图回忆按相关度(库, 关键词, 跳数, 条数上限)`。注入提示词的
  条数是死的，而两跳邻域动辄几十条边，邻域 BFS 一撞上限就停 —— 留下的是先走到的，
  不是最相关的。判据用 qi-graph 的个性化 PageRank（以查询词为种子，边的分数取两端之和）。
  只在候选超过坑位时才跑排序，放得下时输出与 `图回忆` 逐字节相同。代理的自动召回钩子
  （`图关系召回`）已改走这一版。
- **长时程（分段执行 + 断点续跑）**，新模块 `长时程.qi`，从 `Harness` 主入口 re-export。
  `开长任务` / `续长任务` / `设分段` / `加里程碑` / `加里程碑带检查` / `用文件沙箱` /
  `挂预算` / `跑一段` / `一直跑` / `任务进度`。每段开一个**全新**会话，从持久状态
  （目标、里程碑完成情况、上段有界笔记、背景知识）重建上下文，段数再多提示也不变长；
  状态落 SQLite，进程没了下次接着跑。里程碑可挂确定性检查式（`文件存在:` / `文件含:` /
  `图有:`），模型说完成、检查不过就驳回并写进笔记；报完成但里程碑没全达成不许收场。
  预算跨进程接得回来：累计已用令牌落库，`挂预算` 时经新加的 `预算.补记令牌` 补进新账本。
- **知识图当长任务的跨段知识源**：`图记忆.用图记忆(任务, 库路径, 给工具)` 之后每段自动把
  相关事实拼进段提示（读），模型用新工具 `记事实` / `查关系` / `怎么连的` 自己往图里写
  （`装备图记忆工具(代理[, 锚点])`）。锚点让模型记的事实挂在任务节点下，下一段召回
  得回来。依赖方向是 图记忆 → 长时程（`设置图接线`），长时程本身不 import qi-graph。
  `挂图记忆库(代理, 已开句柄)` 给一个进程里很多代理共用同一图库用。
- `图回忆` 省略跳数的便利版默认 **2 跳**（原来 1 跳，等于查找表）；
  `设置图记忆默认跳数` / `图记忆默认跳数`。图的写路径加锁：工具是并发派发的，
  `关联` 里「找已有边 → 加边」不串起来会长出重复边。
- 上下文自动压缩：`运行配置` 默认 `上下文压缩策略 = 1`（滑窗）、阈值 96000，
  `配置摘要压缩` 可切摘要压缩；`滑窗裁剪` 会跳过开头的孤儿 tool 消息。

### Changed

- `最低Qi版本` 抬到 `2026.09.12-2`：`文件工具` 跟着 `列出目录` 的新形状（返回字符串
  列表句柄），`MCP装备` 用 `JSON.取显示值`；再往前的编译器编不过。
- CI / release 钉的 qi / qi-runtime / qi-gui / qi-web 源码 SHA 全部抬到 2026.09.12 发布的那一组。

- **图记忆（第三路记忆：关系型召回）**，新模块 `图记忆.qi`，建在 qi-graph（包名
  `Graph`）的属性图上。词法（`记忆.qi` SQLite + LIKE）和语义（`向量记忆.qi` 余弦）
  都答不了「谁用过这个工具」「这个模块依赖什么」「A 和 B 之间怎么连上的」——
  答案不在任何一条记忆的字面里，而在条目之间的边上。
  存储层：`打开图记忆` / `记实体`（幂等）/ `关联`（两端实体自动建，同一对端点 +
  同一关系幂等）/ `关联带属性` / `图回忆`（逗号分隔关键词，按元数重载出 1 跳与
  N 跳两版，多关键词邻域合并去重、`条数上限` 封顶）/ `怎么连的`（最短路径渲染成
  一行）/ `按关系回忆` / `图记忆条数` / `图记忆统计` / `图记忆可用` / `关闭图记忆`。
  代理层：`启用图记忆(代理值, 路径)` / `代理图记忆库` / `记关联`；`带记忆运行`
  变成**两路合并召回** —— 词法块和关系块拼进同一个 `## 相关记忆与过往经验` 标题块
  （关系那一路发 `graph_recall` 事件，词法仍是 `memory_recall`），两路都可选、
  都没命中时一个字都不注入，与 `运行` 完全等价。
  输出格式与 `记忆.回忆` 对齐（每行 `-【标签】正文`，这里标签是关系名），
  所以三路召回的块可以直接拼在一起丢进提示词。
  qi-graph 的键靠 `:` 分段，而 agent 世界的名字常带冒号（`mcp:fetch`、`用户:1`）：
  内部走安全名（能当键段的原样，不能的整串转十六进制），原名存进节点/边属性，
  查找过安全名、展示回原名，调用方全程只见原名。
- `Harness.代理::设置关系召回器` / `设置代理图记忆库`（+ 代理状态新字段
  `图记忆库`，会话 JSON 键 `graph_memory`）。**依赖方向是 图记忆 → 代理，不是
  反过来**：只要 `代理.qi` 引一下 `图记忆` 的符号，每个用 Harness 的程序（哪怕
  只调 `创建代理`）都得能解析到 qi-graph + qi-kv 才编得过（实测报
  `注册中心依赖 KV 尚未安装`）。所以 `代理.qi` 只留一个不透明句柄槽 + 一个召回
  钩子，接线在 `图记忆.qi` 里做。同理 **`图记忆` 不从 `Harness.qi` re-export**，
  跟 `观测台`（依赖 qi-web）一样要显式 `导入 Harness.图记忆::{...}` ——
  不用这个模块的人完全不受影响。
- `tests/graph_memory/`（存储层 + 代理层两支，后者在 stdout 的 `llm_call` 事件上
  核对 `带记忆运行` 到底注入了什么），已接进 `run-offline-tests.sh`；
  qi-graph / qi-kv 解析不到时套件自己跳过而不是把质量门卡红。
  示例 `examples/图记忆_关系召回.qi`。
