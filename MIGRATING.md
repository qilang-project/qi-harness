...
qi-harness `0.5.x` requires Qi `2026.09.12-2` or newer. `文件工具` follows the new shape of `标准库.操作系统.列出目录` (a string-list handle instead of a newline-joined string) and `MCP装备` uses `标准库.JSON.取显示值`; older compilers cannot compile the package.
